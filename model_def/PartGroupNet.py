...
class PartGroupNet (nn.Module):
    ...
    def init_parameters (self, init_centers=None, init_smooth_factors=None): #the init_param have self calling meaning default can change
        if init_centers is None: # this can be any thing but considered none
            nn.init.kaiming_normal_(self.part_centers)  # used He Initialisation
            self.part_centers.data.clamp_(min=1e-5) # will take numbers above min 
        else:
            # init weight based on clustering  (based on clusters it will initialise weight otherwise already 0)
    ...
    def forward (self, featmaps):
        ...
        x = featmaps.view(bs, ch, -1).contiguous()    # B x 256 x 49(7x7)
        ...
        # C^T x X
        cx_ = torch.bmm(c, x) # B x K x 49 (This function performs the matrix multiplication for each batch independently)
